...
fn get_info(image:&DynamicImage) -> DungeonInfo {
    let clr = image::Rgba([230, 224, 233, 255]);
    for x in 220..378 {
...
pub fn bitmap_from_image(image:&DynamicImage, opt:&Opt) -> Option<Bitmap> {
    ...
    if !opt.no_ocr {
        ...
        bitmap.set_info(get_info(&image));
